...
- `searchDiagnostics` now also includes `governance_rollup` so operators can quickly see visible result status counts and how many returned items still need governance review.
- `searchDiagnostics.retrieval_governance` now reports how many candidates were hidden before return because governance marked them `superseded` or `duplicate`.
